...
function stock_ticker_get_data($stock_list){
	...
	for($i=0;$i<3;$i++){
		...
		if($response_list->_cache_status=="Cache Locked"||$response_list->_cache_status=="Feed Error"||$response_list->_cache_status=="All Failed"){
			sleep(.5);
			continue;
		}
		//This for loop extracts the data from the response and determines what to do with that data
		//Possible cases are: 
